...
--follows table
DECLARE @follower INT = 2;
DECLARE @followee INT;
DECLARE @batchSize INT = 1000;
DECLARE @counter INT = 0;
DECLARE @sql NVARCHAR(MAX) = '';

-- Loop through follower_id 2 to 100
WHILE @follower <= 100
BEGIN
    SET @followee = 1;  -- Start followee from 1
    
    -- Start building the SQL statement for the current batch
    SET @sql = 'INSERT INTO follows (follower_id, followee_id) VALUES ';
    
    -- Build the values part
    WHILE @followee <= 100
    BEGIN
        SET @sql = @sql + '(' + CAST(@follower AS NVARCHAR) + ', ' + CAST(@followee AS NVARCHAR) + ')';
        
        SET @followee = @followee + 1;
        
        -- Add a comma after each value pair unless it's the last one
        IF @followee <= 100
        BEGIN
            SET @sql = @sql + ', ';
        END
    ...
    -- Execute the built SQL statement for the current follower_id
    EXEC sp_executesql @sql;
    
    -- Move to the next follower
    SET @follower = @follower + 1;
END

--imported by converting data into csv format
